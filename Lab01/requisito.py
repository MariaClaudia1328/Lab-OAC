...
if entrada.mode == "r":
    ...
    reg = re.compile("\w+:", flags=re.M)  #label 
    segD = re.compile(".data", flags=re.M) #.data
    segT = re.compile(".text", flags=re.M) #.text
    labelDict = {} 
    instructionsDict = {}
    seg_Data = True
    for i in contents:
...
def teste_word(item):
    ...
    if(is_number(item)):
        ...
        if(type(item) == str and len(item) == 1):
            print(f'.word recebeu char: {item}')
            sys.exit()
        if(type(item) == str and '0x' not in item):
            print(f'.word recebeu string mal formatada: {item}')
            sys.exit()

def teste_byte(item):
    item = item.replace(",", "")
    if(type(item) == str and len(item) != 3):
        print(".byte não recebeu char")
        sys.exit()
...
for s in saida_data:
    if is_number(s):
        saida_data_.append(s)
    else:
        if (type(s) == str):
            if("0x" in s):
                new_v = (int(s,16))
                saida_data_.append(new_v)
            else:
                new_v = (int(s,2))
                saida_data_.append(new_v)
        else:
            print("erro n constatado")
    


addres_data = []
index = 0
for add in saida_data_:
    addres_data.append(np.base_repr(index,base=16).rjust(8,'0'))
    saida_data_[index] = np.base_repr(int(add), base = 16).rjust(8,'0')
    index += 1
...
#escrever em .data
for value,add in zip(saida_data_,addres_data):
    s = (add) + ' : ' + value + ';\n'
...
for info in list_isntructions:
    ...
    if splitt[0] in r_dict:
        ...
        if(splitt[0] == '.byte' or splitt[0] == '.word'):
            typee = "UNDEFINED TYPE"
        else:
            print(f'Instrução não encontrada: {splitt[0]}')
            sys.exit()
...
for i in list_isntructions_types:
    for j in i:
        ...
        if(j == 'imm'):
            ...
            elif(i[j] not in labelDict and ((i[j][0] > 'A' and i[j][0] < 'Z' ) or (i[j][0] > 'a' and i[j][0] < 'z' ))):
                print("Não foi possível executar: campo imediato de ", i["instruction"], "inválido. Execução finalizada")
                sys.exit()
...
for i,j,k in zip(address_,saida, instru):
    s = i + ' : ' + j +';'+' % '+k+' '+'%'+'\n'
